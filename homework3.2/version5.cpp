...
void simdneon(int n){ //普通并行
    ...
    std::cout<<"total: "<<time2<<" num: "<<num<<" simdneon: "<<time2/num<<std::endl;
    time2=time2/num;
}
...
void *threadfunc(void *param){
    ...
    for(int k=0;k<n;k++){   //除法操作
        ...
        int sectionnum=(n-k-1)/(num_thread-1);
        int ed=(t_id==(num_thread-1)?n:k+1+sectionnum*(t_id+1));
        for(int i=k+1+sectionnum*t_id;i<ed;i++){    //处理消去操作时，每个线程以8行为间隔处理某一行
            float32x4_t vaik=vld1q_dup_f32(m1[i]+k);
               int j;
               for(j=k+1;j+4<=n;j+=4){
                   float32x4_t vakj=vld1q_f32(m1[k]+j);
