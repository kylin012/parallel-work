...
void sse(int n){ 
    ...
    while(duration_cast<duration<double>>(high_resolution_clock::now() - t1).count()<1){
      for(int k=0;k<n;k++){
          ...
          for(;j+4<=n;j+=4){
              __m128 va =_mm_load_ps(m1[k]+j);
              va=_mm_div_ps(va,vt);
              _mm_store_ps(m1[k]+j,va);
          }
          for(;j<n;j++)
          ...
          for(int i=k+1;i<n;i++){
              ...
              for(;j+4<=n;j+=4){
                  __m128 vakj=_mm_load_ps(m1[k]+j);
                  __m128 vaij=_mm_load_ps(m1[i]+j);
                  __m128 vx= _mm_mul_ps(vaik,vakj);
                  vaij = _mm_sub_ps(vaij,vx);
                  _mm_store_ps(m1[i]+j,vaij); 
              }
              for(;j<n;j++)
...
void avx(int n){
    ...
    while(duration_cast<duration<double>>(high_resolution_clock::now() - t1).count()<1){
      for(int k=0;k<n;k++){
          ...
          for(;j+8<=n;j+=8){
              __m256 va =_mm256_load_ps(m1[k]+j);
              va=_mm256_div_ps(va,vt);
              _mm256_store_ps(m1[k]+j,va);
          }
          for(;j<n;j++)
              m1[k][j]=m1[k][j]/m1[k][k];
          m1[k][k]=1.0;
          for(int i=k+1;i<n;i++){
              ...
              int j;
              if((k+1)%8!=0){
                  for(j=k+1;j<(k+9)/8*8;j++){
                       m1[i][j]-=m1[i][k]*m1[k][j];
                  }
              }
              for(;j+8<=n;j+=8){
                  __m256 vakj=_mm256_load_ps(m1[k]+j);
                  __m256 vaij=_mm256_load_ps(m1[i]+j);
                  __m256 vx= _mm256_mul_ps(vaik,vakj);
                  vaij = _mm256_sub_ps(vaij,vx);
                  _mm256_store_ps(m1[i]+j,vaij); 
              }
              for(;j<n;j++)
                  m1[i][j]=m1[i][j]-m1[i][k]*m1[k][j];
              m1[i][k]=0;
...
//          }
//           for(;j+16<=n;j+=16){
//               __m512 va =_mm512_load_ps(m1[k]+j);
//               va=_mm512_div_ps(va,vt);
//               _mm512_store_ps(m1[k]+j,va);
//           }
//           for(;j<n;j++)
//               m1[k][j]=m1[k][j]/m1[k][k];
//           m1[k][k]=1.0;
...
//              }
//               for(;j+16<=n;j+=16){
//                   __m512 vakj=_mm512_load_ps(m1[k]+j);
//                   __m512 vaij=_mm512_load_ps(m1[i]+j);
//                   __m512 vx= _mm512_mul_ps(vaik,vakj);
//                   vaij = _mm512_sub_ps(vaij,vx);
//                   _mm512_store_ps(m1[i]+j,vaij); 
//               }
//               for(;j<n;j++)
//                   m1[i][j]=m1[i][j]-m1[i][k]*m1[k][j];
//               m1[i][k]=0;
...
int main(){
    ...
    //reset(n);
    // avx512(n);
    // std::cout<<"serial/avx512: "<<ttime/time4<<std::endl;
